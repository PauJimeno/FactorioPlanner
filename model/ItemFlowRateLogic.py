...
class ItemFlowRateLogic(DirectionalElement, GridElement):
    ...
    def variable_input_rate(self):
        ...
        :rtype: Array
        """
        return [And(self.input_flow_rate[coord[0]][coord[1]] >= 0,
                    self.input_flow_rate[coord[0]][coord[1]] <= self.max_rate) for
                coord in self.input]
            ...
            for j in range(self.width):
                ...
                if not self.is_input(i, j):
                    belt_flow_rate_propagation.append(Implies(self.conveyor[i][j] != self.direction[0],
                                                              And(self.input_flow_rate[i][j] == sum(belt_input_cells),
                                                                  self.input_flow_rate[i][j] <= self.max_rate)))
                # La sortida d'una cinta és la seva entrada menys la suma d'entrada de les caselles adjacents que
                # prenen items
                belt_flow_rate_propagation.append(Implies(self.conveyor[i][j] != self.direction[0],
                                                          And(self.output_flow_rate[i][j] == (
                                                                  self.input_flow_rate[i][j] - sum(
                                                              belt_output_cells)),
    ...
    def inserter_item_flow_propagation(self):
        """
        Creates a constraint that encodes the behaviour of how the amount of items an inserter
        carries is propagated.
        ...
        The output flow rate of the inserter will always be the same as the input.
            ...
            for j in range(self.width):
                if not self.is_input(i, j):
                    ...
                    for direction in range(1, self.n_dir):
                        x, y = i + self.displacement[direction][0], j + self.displacement[direction][1]
                        if 0 <= x < self.height and 0 <= y < self.width:
                            # L'entrada d'un inserter ha de ser mes petita o igual a 50 en cas que l'entrada de la
                            # casella d'on agafa items li entrin 50 o més items
                            inserter_input_cells.append(Implies(And(self.inserter[i][j] == self.opposite_dir[direction],
                                                                    self.input_flow_rate[x][y] >= 50),
                                                                And(self.input_flow_rate[i][j] <= 50,
                                                                    self.input_flow_rate[i][j] ==
                                                                    self.output_flow_rate[i][j])))
                            # L'entrada d'un inserter ha de ser mes petita o igual a l'entrada de la casella d'on agafa
                            # si l'entrada d'aquesta és mes petita que 50
                            inserter_input_cells.append(Implies(And(self.inserter[i][j] == self.opposite_dir[direction],
                                                                    self.input_flow_rate[x][y] < 50,
                                                                    self.route[x][y] != 0),
                                                                # La ruta ha de ser != 0 (no esta agafant d'un
                                                                # assembler)
                                                                And(self.input_flow_rate[i][j] <=
                                                                    self.input_flow_rate[x][y],
                                                                    self.output_flow_rate[i][j] ==
                                                                    self.input_flow_rate[i][j])))
                            inserter_input_cells.append(Implies(And(self.inserter[i][j] == self.opposite_dir[direction],
                                                                    self.route[x][y] == 0), And(
                                self.input_flow_rate[i][j] == self.output_flow_rate[i][j],
                                self.input_flow_rate[i][j] <= 50)))

                    inserter_item_flow_propagation.append(Implies(self.inserter[i][j] != self.direction[0],
                                                                  And(And(inserter_input_cells),
                                                                      self.input_flow_rate[i][j] > 0)))
        return inserter_item_flow_propagation

    def item_loss(self):
        """
        Sums the number of items that are not getting used by assemblers (loss),
        the loss happens when the flow rate of a conveyor feeding an inserter feeding an assembler
        is greater than 0.

        :return: sum of unused items, used for optimization
        :rtype: Array
        """
        loss = []
        for i in range(self.height):
            for j in range(self.width):
                for direction in range(1, self.n_dir):
                    ...
                    if 0 <= x < self.height and 0 <= y < self.width:
                        ...
                        loss.append(If(And(self.conveyor[i][j] != self.direction[0],
                                           self.inserter[x][y] == self.conveyor[i][j]), self.output_flow_rate[i][j], 0))
        return sum(loss)
