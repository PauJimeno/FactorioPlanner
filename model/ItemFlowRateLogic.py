...
class ItemFlowRateLogic(DirectionalElement, GridElement):
    ...
    def domain_constraint(self):
        return[If(self.route[i][j] != 0, And(self.item_flow_rate[i][j] >= 0, self.item_flow_rate[i][j] <= 450), self.item_flow_rate[i][j] == 0)
    ...
    def constraints(self):
        return self.item_input_rate() + self.belt_item_flow_propagation() + self.inserter_item_flow_propagation() + self.domain_constraint()
